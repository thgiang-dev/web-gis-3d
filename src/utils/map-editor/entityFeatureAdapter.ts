...
export function spatialFeatureToEditableGraphic(
  ...
  return new Graphic({
    ...
    attributes: {
      ...
      SIZE_X: feature.model3D.scale?.x ?? 5,
      SIZE_Y: feature.model3D.scale?.y ?? 5,
      SIZE_Z: feature.model3D.scale?.z ?? 5,
...
export function updateEntityFromEditedFeature(
  ...
  const sizeX = normalizePositiveNumber(attributes.SIZE_X ?? attributes.SIZE, entity.model3D?.scale?.x ?? 5)
  const sizeY = normalizePositiveNumber(attributes.SIZE_Y ?? attributes.SIZE, entity.model3D?.scale?.y ?? 5)
  const sizeZ = normalizePositiveNumber(attributes.SIZE_Z ?? attributes.SIZE, entity.model3D?.scale?.z ?? 5)
